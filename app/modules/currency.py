...
def fetch_currencies(symbols = ('USD','RUB','GBP')):
    # Для простоты: получаем курсы к EUR из ECB (последние значения)
    try:
        r = requests.get(ECB_URL, timeout=10)
        if r.status_code != 200:
            return None
        data = r.json()
        # Выборка упрощена: находим последние значения для каждой валюты
        ...
        for key, series in series_list.items():
            # key like '0:%d:0' where second number is currency idx
            parts = key.split(':')
            if len(parts) < 2: 
                continue
            cidx = int(parts[1])
